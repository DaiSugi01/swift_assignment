...
class ViewController: UIViewController {
    ...
    private func scaleNavBarHeight() {
        UIView.animate(withDuration: 1.0, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.3, animations: {
            if self.isNavbarHideen {
                ...
                let translateTitle = CGAffineTransform(translationX: self.topView.frame.width / 2 - 190, y: self.topView.bounds.size.height / 2 - 30)
                ...
                self.tableView.contentInset = UIEdgeInsets(top: self.topView.bounds.height + 15, left: 0, bottom: 0, right: 0)
            } else {
